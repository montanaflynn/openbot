...
/// Command output: dim indented text with "  │ " prefix.
pub fn styled_cmd_output(text: &str) -> Line<'static> {
    Line::from(vec![
        Span::styled("  │ ", Style::default().fg(Color::DarkGray)),
...
/// "— Worked for Xs —" reasoning separator.
pub fn styled_worked(duration: std::time::Duration) -> Line<'static> {
    let secs = duration.as_secs();
    let text = if secs >= 60 {
        format!("— Worked for {}m {}s ", secs / 60, secs % 60)
    } else {
        format!("— Worked for {}s ", secs)
    };
    let pad = 60usize.saturating_sub(text.len());
    Line::from(Span::styled(
